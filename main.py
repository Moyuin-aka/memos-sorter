...
def main():
    ...
    try:
        ...
        if all_memos:
            ...
            api_thread = threading.Thread(target=classify_worker, args=(markdown_content, user_categories))
            api_thread.start()

            loading_animation(api_thread)
...
def classify_worker(markdown_content, user_categories):
    """在工作线程中调用 Gemini API。"""
    global g_classified_memos, g_api_error
    try:
